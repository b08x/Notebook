        ...
        switch (res.headers["content-encoding"]) {
          ...
          case "br":
            if (isBrotliSupported) {
              streams.push(import_zlib.default.createBrotliDecompress(zlibOptions));
              delete res.headers["content-encoding"];
            }
        }
      }
...
var ArchiveBoxPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    ...
    this.loggedIn = false;
    this.isNodeAxios = false;
    this.nodeAxiosSession = "";
  }
  async onload() {
    ...
    this.registerEvent(this.app.vault.on("modify", async (file) => {
      if (this.settings.autoSubmitOnModify === true) {
        if (this.getFileExtension(file.path) === ".md") {
          const data = await this.app.vault.adapter.read(file.path);
          this.archiveURLSInData(data);
        }
      }
    }));
    this.registerEvent(this.app.workspace.on("file-menu", (menu, file) => {
      if (this.getFileExtension(file.path) === ".md") {
        menu.addItem((item) => {
          item.setTitle("ArchiveBox links...").setIcon("document").onClick(async () => {
            const data = await this.app.vault.adapter.read(file.path);
            this.archiveURLSInData(data, true);
          });
        });
      }
    }));
  ...
  async saveSettings(unload = false) {
  ...
  validateSettings(updateStatusBar = true) {
    if (this.settings.debugMode === true) {
      console.log("Validating settings.");
    }
    try {
      new import_url2.URL(this.settings.archiveBoxURI);
    } catch (e) {
      if (updateStatusBar) {
    ...
    if (this.settings.username === "" && this.settings.password !== "" || this.settings.password === "" && this.settings.username !== "") {
      if (updateStatusBar) {
  ...
  async archiveURLSInData(data, immediate = false) {
    const rexp = /\[[^\(\)]+\]\(([^\[\]\(\)]+)\)/g;
    let links = [];
    let match;
    while (match = rexp.exec(data)) {
      try {
        ...
        let url2 = new import_url2.URL(match[1]);
        ...
        if (this.settings.ignoreRFC1918Addresses) {
          if (this.isRfc1918Address(url2)) {
            console.log(`Ignoring RFC1918 address ${url2.hostname}`);
            continue;
          }
        }
        if (this.settings.ignoreDomains !== "" && this.isIgnoredDomain(url2)) {
          console.log(`Ignoring ${url2.hostname} as it is in the ignored domains list`);
          continue;
        }
  ...
  async submitToArchiveBox(links, forceUpdate = false) {
    if (this.loggedIn === false) {
      console.log(`ArchiveBox: Getting session...`);
      this.updateStatusBar("Logging into ArchiveBox...");
      await this.loginToArchiveBox(this.settings.archiveBoxUsername, this.settings.archiveBoxPassword);
    }
    this.linkBatch = this.linkBatch.concat(links);
    if (forceUpdate === true || this.linkBatch.length > 0 && (this.lastLinkUpdate === 0 || this.lastLinkSubmit === 0 || this.lastLinkUpdate - this.lastLinkSubmit > this.settings.batchEverySec * 1e3)) {
  ...
  async initializeAxiosClient() {
    ...
    this.loggedIn = false;
    this.axios = axios_default.create(configOptions);
  }
  async loginToArchiveBox(username, password) {
    if (this.settings.debugMode === true) {
  ...
  async addUrlsToArchiveBox(urls) {
    try {
      let config = {
        timeout: 2e3
      };
      ...
      let response = await this.axios.post("/add/", {
        url: urls.join("\n"),
        parser: "url_list",
        tag: "obsidian",
        depth: "0"
  ...
  isRfc1918Address(url2) {
    ...
    return rfc1918Addresses.some((range) => {
      ...
      if (octets.length !== baseOctets.length || !octets.every((octet, i) => octet === baseOctets[i])) {
        return false;
      }
      const bits = Number(prefix);
      return octets.slice(0, bits / 8).every((octet) => octet === baseOctets[bits / 8]);
    });
  }
  ...
  isIgnoredDomain(url2) {
    const domain = url2.hostname;
    const domainList = this.settings.ignoreDomains.split(",").map((s) => s.trim());
...
var ArchiveBoxSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    const ArchiveBoxPluginSettings = [
      {
        name: "ArchiveBox URI",
      ...
      },
      {
        ...
        type: "boolean",
        description: "Ignore private addresses in URIs (e.g. https://192.168.1.1)"
      },
      {
      ...
      },
      {
        ...
        type: "boolean",
        description: "Use HTTP Basic Authentication in front of ArchiveBox."
      },
      {
      ...
      },
      {
        ...
        type: "number",
        description: "How often to send detected links to ArchiveBox via auto-submit.",
        placeholder: "5"
      },
      {
        name: "Cache URIs",
    ...
    const self2 = this;
    ArchiveBoxPluginSettings.forEach((setting) => {
      let settingsObject = new import_obsidian.Setting(containerEl).setName(setting.name).setDesc(setting.description);
      switch (setting.type) {
        case "text":
          settingsObject.addText((text) => {
            text.setPlaceholder(setting.placeholder ? setting.placeholder : "").setValue(self2.plugin.settings[setting.settingsKey]).onChange(async (value) => {
              self2.plugin.settings[setting.settingsKey] = value;
        ...
        case "number":
          settingsObject.addText((text) => {
            text.setPlaceholder(setting.placeholder ? setting.placeholder : "").setValue(self2.plugin.settings[setting.settingsKey]).onChange(async (value) => {
              self2.plugin.settings[setting.settingsKey] = parseInt(value);
              await self2.plugin.saveSettings();
            });
          });
          break;
        case "boolean":
          settingsObject.addToggle((component) => {
            component.setValue(self2.plugin.settings[setting.settingsKey]).onChange(async (value) => {
              self2.plugin.settings[setting.settingsKey] = value;
              await self2.plugin.saveSettings();
            });
          });
